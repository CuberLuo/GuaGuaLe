...
function RedeemArea({ history, redeemCode }: { history: { money: string; time: string }[]; redeemCode: string }) {
  ...
  const autoCopy = useCallback(async () => {
    ...
    setToastMsg(success ? '已复制兑奖码，请发送给罗俊涛进行兑奖' : '自动复制失败，请手动复制兑奖码');
  }, [redeemCode]);
  ...
  useEffect(() => {
    ...
    if (codeRef.current) {
      gsap.fromTo(codeRef.current,
        { y: 30, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.8, ease: 'power2.out', delay: 1.5, onComplete: autoCopy }
      );
    }
  ...
  return (
    <div className="redeem-area" ref={containerRef}>
      ...
      <div className="redeem-total">
        ...
        <span className="redeem-amount" ref={amountRef}>¥0.00</span>
      </div>
